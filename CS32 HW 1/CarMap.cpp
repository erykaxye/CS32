...
// and there is room in the map, add an entry fo that car recording
...
{
	if (!m_map.get(license, gallons) || gallons < 0)
		return false;
	m_map.update(license, gas(license) + gallons); //maybe? 
	return true;
}
...
// true.
{
	if (!m_map.get(license, gallons) || gallons < 0 || gallons > gas(license))
		return false;
	m_map.update(license, gas(license) - gallons); //maybe? 
	return true;
}
...
// Write to cout one line fo every car in the map.  Each line
...
// no other text.
{
	for (int i = 0; i < fleetSize(); i++)
	{
		std::string ke = "";
		double va = 0.0;
		cout << m_map.get(i, ke, va) << " " << gas(ke) << " " << endl; 
	}
}
