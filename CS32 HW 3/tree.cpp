...
{
	if (n1 <= 0 && n2 <= 0) //do we need the first part to check? 
		return 1; 
	if (n1 == 0 && n2 != 0)
		return 0; 
	if (a2[0] == a1[0])
...
//(Hint: Using the split function, the order function can be 
//written in fewer than eight short lines of code.)
...
void order(double a[], int n)
{
	...
	order(a, firstNotGreater); //do the first half
	order(a + firstLess, n - firstLess); //start at postion of firstless and n would be the number of elements left
}
